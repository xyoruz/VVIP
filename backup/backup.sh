...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/xyoruz/izinvps/ipuk/ip | grep $MYIP | awk '{print $2}')
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "xyr3760@gmail.com" &> /dev/null
    git config --global user.name "xyoruz" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M ipuk &> /dev/null
    git remote add origin https://github.com/xyoruz/backup
    git push -f https://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@github.com/xyoruz/backup.git &> /dev/null
}
